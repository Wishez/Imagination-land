...
},{"./slider.js":3,"jquery":5}],3:[function(require,module,exports){
'use strict';
...
var _normwheel = require('./lib/normwheel.js');
...
(function (factory) {
	if (typeof define === 'function' && define.amd) {
		...
		define(['jquery', _gsap.TimelineMax, _normwheel2.default], factory);
	} else if ((typeof exports === 'undefined' ? 'undefined' : _typeof(exports)) === 'object') {
		// Common JS

		module.exports = factory(require('jquery'), _gsap.TimelineMax, _normwheel2.default);
	} else {
		// Globals
		window.SmoothSlider = factory(require('jquery'), _gsap.TimelineMax, _normwheel2.default);
	}
	...
	var init = function init() {
		...
		var $slider = $(props.sliderId).css({
			position: 'relative',
			overflow: 'hidden',
			paddingLeft: '1%'
		});
		// Выборка треков.
		var $allTracks = $slider.find('.slider-track');
		/* 
   ...
   */
		var tracks = [];
		// Компонуются данные
		$allTracks.each(function (i, e) {

			tracks[i] = {};
			var currentTrack = tracks[i];
			currentTrack.$track = $(e);
			var $currentChildren = currentTrack.$track.children();
		...
		var _state = {
			...
			tl: new _gsap.TimelineMax(), // Глобальная сцена переходов.
			paddingLeft: 1 // Для лучего отображения анимации.
		};
		// Меняет ширину трека, когда исчезают слайды при прокрутке.
		function _changeTrack(track, currentSlide) {
			var reverse = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

			// Слайд, который скроется при прокрутке
			var currentChildren = track.$track.children();
			var $firstHiddenSlide = $(currentChildren[currentSlide]).find(':first-child');

			var removeClass = '';
			var addClass = '';
			var isAppend = true;
			var leftTransition = imageWidth / 2 + 20; // Переменная для плавного сокрытия слайда. 
			...
			$firstHiddenSlide.addClass(addClass).removeClass(removeClass);

			new TweenLite().to($firstHiddenSlide, 0.1, {
				left: leftTransition
			});

			var $copyHiddenSlide = $firstHiddenSlide.parent().clone();
			$copyHiddenSlide.find(':first-child').addClass('slide_shown').removeClass('slide_hidden');
		...
		$slider.on('wheel', function (e) {
			e.preventDefault();
			var norm = NormalizeWheel(e.originalEvent);
			var spinY = norm.spinY;
			...
			_state.tl.add(new TweenLite().to($allTracks, 1, {
				left: _state.currentPosition
			}));
		});
	};
...
},{"./lib/normwheel.js":1,"gsap":4,"jquery":5}],4:[function(require,module,exports){
(function (global){
/*!
 * VERSION: 1.20.3
 * DATE: 2017-10-02
